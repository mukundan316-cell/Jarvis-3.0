...
export function QuickCommandPanel({ onCommand, disabled = false }: QuickCommandPanelProps) {
  const [input, setInput] = useState('');
  const { currentPersona: detectedPersona } = usePersona();
  ...
  // Fetch persona-specific quick commands from ConfigService
  const { data: quickCommandsConfig, isLoading: commandsLoading } = useQuery({
    queryKey: ['/api/config/setting/quickcommands.config', { persona: detectedPersona }],
    queryFn: () => 
      fetch(`/api/config/setting/quickcommands.config?persona=${detectedPersona}`)
        .then(res => res.json()),
    ...
    <Card className="bg-slate-900/50 border-blue-500/30">
      <CardHeader>
        <CardTitle className="text-white flex items-center space-x-2">
          ...
          <span>Rachel's Command Center</span>
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <form onSubmit={handleSubmit} className="flex gap-2">
          <Input
            ...
            placeholder="Type a command or use quick buttons below"
        ...
        <div className="space-y-2">
          <p className="text-gray-400 text-sm">Quick Commands:</p>
          {commandsLoading ? (
            <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 gap-2">
              {quickCommands.map((cmd) => (
                <motion.button
                  ...
                  data-testid={`button-quickcommand-${cmd.text.toLowerCase().replace(/\s+/g, '-')}`}
                >
