...
interface UniversalRecentActivitiesProps {
  persona?: PersonaType;
  showPersonaFilter?: boolean;
}

export function UniversalRecentActivities({ persona, showPersonaFilter = false }: UniversalRecentActivitiesProps) {
  // Standardized Universal query pattern - prevents cache fragmentation
  const { data: activities = [], isLoading } = useQuery({
    queryKey: ['/api/activities'], // Simplified Universal pattern
    refetchInterval: 30000, // Refresh every 30 seconds
  });

  // Get status colors configuration from ConfigService
  const { statusColors, statusTextColors, isLoading: configLoading } = useStatusColorsConfig(persona);
